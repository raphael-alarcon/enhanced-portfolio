            ...
            {steps.map((step, index) => (
                <div className={`flex flex-row items-start h-${height} gap-6`} key={index}>
                    <div className="flex flex-col items-center pt-6">
                        <div className={`rounded-full w-5 h-5 relative ${color}`}>
                            {steps[0] == step && <div className={`rounded-full absolute top-0 left-0 animate-ping w-full h-full  ${color}`}/>}
                        </div>
                        <div className={`h-28 w-1 ${steps[steps.length-1] != step ? color : "bg-none"}`}/>
                    </div>
